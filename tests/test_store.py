...
def test_create_job(mock):
    if mock:
        ...
        Job(pdb=PDB, ff=FF, box=BOX, water=WATER, hotkeys=['a','b','c','d'], created_at=time.time(), updated_at=time.time())
...
def test_insert_single_job_into_store(mock):
    ...
    info = {'pdb': PDB, 'ff': FF, 'box': BOX, 'water': WATER, 'hotkeys': ['a','b','c','d'], 'created_at': time.time(), 'updated_at': time.time()}
    if mock:
        job = MockJob()
    else:
        job = Job(**info)
...
async def test_job_update_in_store():
    ...
    with sqlite3.connect(store.db_file) as conn:
        ...
        cursor = conn.cursor()
        cursor.execute(f"SELECT best_loss FROM {store.table_name} WHERE pdb = ?", (job.pdb,))
        stored_loss = cursor.fetchone()['best_loss']
    
    assert stored_loss == new_loss, f"stored loss should be {new_loss}, got {stored_loss}"
...
def test_store_handles_complex_data_types():
    ...
    assert retrieved_job.system_kwargs == job.system_kwargs, "Complex system_kwargs not preserved"
