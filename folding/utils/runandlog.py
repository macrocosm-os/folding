    ...
    def run_cmd_commands(
        ...
        for cmd in tqdm.tqdm(commands): # set tuple 
            ...
            try:
                ...
                command_dict = self.command_dict # we have the option of using the command dict before exception is raised

                # Raise an exception during a failure event
                raise subprocess.CalledProcessError(returncode=f"Failed to run command: {cmd}")
...
test_commands = ['cd /home/spunion/folding/data/8emf/deliverable1/no_seed_1', gmx_command_1]
