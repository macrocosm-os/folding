...
class BaseReward(ABC):
    ...
    async def apply(self, data: BatchRewardInput) -> RewardEvent:
        ...
        batch_rewards_output.rewards = await self.calculate_final_reward(rewards=batch_rewards_output.rewards, job=data.job)
    ...
    async def calculate_final_reward(self, rewards: torch.Tensor, job: Job) -> torch.Tensor:
